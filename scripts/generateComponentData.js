...
let chalk = require('chalk');//color the command output
let parse = require('react-docgen').parse;
let chokidar = require('chokidar');//watch file and run a function in a cross platform way

let paths = {
  examples: path.join(__dirname, '../src', 'docs', 'examples'),
  components: path.join(__dirname, '../src', 'components'),
  dragAndDropComponents: path.join(__dirname, '../src/components', 'DragAndDrop'),
...
if (enableWatchMode) {
  ...
  chokidar.watch([paths.examples, paths.components, paths.dragAndDropComponents]).on('change', function(event, path) {
    generate(paths);
  });
} else {
